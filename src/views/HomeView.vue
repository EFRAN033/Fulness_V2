<template>
    ...
    <nav class="w-full">
      <div class="flex justify-between items-center">
        ...
        <a href="#" class="flex items-center h-14 overflow-hidden">
          <img src="@/assets/imagenes/LOGO.svg" alt="Fullness Logo"
            class="h-12 translate-y-2 object-contain hover:scale-105 transition-transform duration-300">
        </a>

        <!-- Menú Hamburguesa (Mobile) -->
        <button @click="isMenuOpen = !isMenuOpen" class="md:hidden p-2 text-[#104e75] focus:outline-none">
        ...
        <div class="hidden md:flex gap-6 md:gap-12 items-center">
          ...
          <div class="flex gap-4 ml-8">
            ...
            <div v-else class="flex items-center gap-4">
              <div @click="logout" class="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity">
                <div class="w-8 h-8 bg-[#104e75] rounded-full flex items-center justify-center text-white">
      ...
      <div v-show="isMenuOpen"
        ...
        :class="isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'" @click="toggleMenu">
        <div
          ...
          <div class="h-full flex flex-col">
            ...
            <div class="px-6 py-5 border-b border-[#104e75]/10 flex items-center justify-between">
              <img src="@/assets/imagenes/LOGO.svg" alt="Logo" class="h-10 w-auto" />
              ...
              <div class="space-y-4">
                ...
                <div v-else class="space-y-3">
                  <div @click="logout"
                    ...
                    <div class="w-10 h-10 bg-[#104e75] rounded-full flex items-center justify-center text-white">
                      <i class="fas fa-user"></i>
                    </div>
                    <div>
  ...
  <div class="relative w-full h-screen overflow-hidden">
    <img src="@/assets/imagenes/imagen-de-fondo.jpg" alt="Imagen de Fondo" class="w-full h-full object-cover">
    ...
    <div class="absolute inset-0 bg-black/40 flex items-center px-4 md:px-12">
      <div class="text-white max-w-2xl space-y-8">
        <h1 class="text-4xl md:text-5xl font-bold leading-tight">
          ...
          <span class="bg-gradient-to-r from-[#448ba9] to-[#87CEEB] bg-clip-text text-transparent">en tu
            Domicilio</span>
        </h1>
    ...
    <div class="hidden md:flex absolute inset-0 justify-end items-center px-4 md:px-12">
      <form @submit.prevent="guardarCita"
        class="bg-white p-6 rounded-xl shadow-2xl w-[90%] max-w-md backdrop-blur-sm border border-white/20 relative">
        <!-- Badge -->
        <div
          class="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#003157] text-white px-4 py-1 rounded-full text-sm font-medium shadow-lg flex items-center">
        ...
        <div class="space-y-4">
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input v-model="formData.apellido" type="text"
                class="w-full px-3 py-2.5 border-2 border-[#dee3e9] rounded-lg placeholder-[#448ba9] text-sm focus:border-[#003157] focus:ring-2 focus:ring-[#b7ebfa] transition-all outline-none"
          ...
          <div>
            ...
            <div class="grid grid-cols-2 gap-2">
              <button type="button" @click="formData.tipo_paciente = 'nuevo'" :class="[
                ...
                <svg v-if="formData.tipo_paciente === 'nuevo'" class="w-4 h-4 mr-1 text-[#003157]" fill="none"
              ...
              <button type="button" @click="formData.tipo_paciente = 'antiguo'" :class="[
                'text-sm py-2 px-3 rounded-lg transition-all font-medium flex items-center justify-center',
                formData.tipo_paciente === 'antiguo'
                  ? 'border-2 border-[#003157] bg-[#e3f6fd] text-[#003157] shadow-inner'
                  : 'border-2 border-[#dee3e9] bg-white hover:border-[#448ba9]'
              ]">
                <svg v-if="formData.tipo_paciente === 'antiguo'" class="w-4 h-4 mr-1 text-[#003157]" fill="none"
                  stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                Paciente Antiguo
            ...
            <p v-if="!formData.tipo_paciente" class="text-red-500 text-xs mt-1">* Debes seleccionar un tipo de paciente
            </p>
          </div>
          ...
          <div>
            ...
            <input v-model="formData.celular" type="tel"
              class="w-full px-3 py-2.5 border-2 border-[#dee3e9] rounded-lg placeholder-[#448ba9] text-sm focus:border-[#003157] focus:ring-2 focus:ring-[#b7ebfa] transition-all outline-none"
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select v-model="formData.servicio"
                class="w-full px-3 py-2.5 border-2 border-[#dee3e9] rounded-lg text-sm focus:border-[#003157] focus:ring-2 focus:ring-[#b7ebfa] transition-all outline-none appearance-none bg-[url('data:image/svg+xml;base64,')] bg-no-repeat bg-[right:0.75rem_center] bg-[length:1em]">
    ...
    <div v-show="showForm" class="md:hidden fixed inset-0 bg-black/50 z-[2000] flex items-center justify-center p-4"
      ...
      <form class="bg-white p-6 rounded-xl w-full max-w-md max-h-[90vh] overflow-y-auto relative">
        ...
        <!-- Botón cerrar -->
        <button @click="showForm = false" class="absolute top-4 right-4 text-[#448ba9] hover:text-[#003157]">
        ...
        <div class="space-y-4">
          ...
          <div>
            <label class="block text-xs font-semibold text-[#104e75] mb-1">Tipo de Paciente *</label>
            <div class="grid grid-cols-2 gap-2">
              <button type="button" @click="selectPatientType('nuevo')" :class="[
                'text-sm py-2 px-3 rounded-lg transition-all font-medium flex items-center justify-center',
                selectedPatientType === 'nuevo'
                  ? 'border-2 border-[#003157] bg-[#e3f6fd] text-[#003157] shadow-inner'
                  : 'border-2 border-[#dee3e9] bg-white hover:border-[#448ba9]'
              ]">
                <svg v-if="selectedPatientType === 'nuevo'" class="w-4 h-4 mr-1 text-[#003157]" fill="none"
                  stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                Nuevo Paciente
              ...
              <button type="button" @click="selectPatientType('antiguo')" :class="[
                'text-sm py-2 px-3 rounded-lg transition-all font-medium flex items-center justify-center',
                selectedPatientType === 'antiguo'
                  ? 'border-2 border-[#003157] bg-[#e3f6fd] text-[#003157] shadow-inner'
                  : 'border-2 border-[#dee3e9] bg-white hover:border-[#448ba9]'
              ]">
                <svg v-if="selectedPatientType === 'antiguo'" class="w-4 h-4 mr-1 text-[#003157]" fill="none"
                  stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                Paciente Antiguo
    ...
    <div class="absolute inset-x-0 bottom-0 h-24 -mb-1">
      <svg viewBox="0 0 1440 120" class="w-full h-full text-[#b7ebfa]/20">
        <path fill="currentColor"
          d="M0,96L48,90.7C96,85,192,75,288,74.7C384,75,480,85,576,106.7C672,128,768,160,864,160C960,160,1056,128,1152,106.7C1248,85,1344,75,1392,69.3L1440,64L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z">
        </path>
      </svg>
    </div>
        ...
        <div class="p-6 space-y-5">
          ...
          <div>
            ...
            <textarea rows="3"
              ...
              placeholder="Ej. Dolor de espalda, lesión deportiva..."></textarea>
          </div>

          <button
            class="w-full bg-gradient-to-r from-[#003157] to-[#104e75] text-white py-2.5 rounded-lg font-medium hover:from-[#00203a] hover:to-[#0d3a5d] transition-all flex items-center justify-center gap-2">
      ...
      <div class="grid lg:grid-cols-2 gap-7 lg:gap-9 items-start">
        ...
        <div class="text-center lg:text-left space-y-8">
          <div class="space-y-6">
            ...
            <div
              class="inline-flex items-center bg-white px-5 py-3 rounded-lg shadow-sm border border-[#b7ebfa] hover:border-[#448ba9] transition-colors">
          ...
          <div class="bg-gradient-to-br from-[#104e75] to-[#003157] rounded-xl p-5 shadow-lg">
            ...
            <div class="mt-4 pt-4 border-t border-[#b7ebfa]/30 flex items-center">
              ...
              <span class="text-xs md:text-sm font-medium text-[#b7ebfa]">Cobertura: Jesús María, San Isidro,
                Miraflores</span>
            </div>
          </div>
          ...
          <div class="group transition-transform hover:scale-[1.02]">
            <a href="#video-tutorial" target="_blank"
              class="inline-flex items-center w-full bg-[#b7ebfa] hover:bg-[#104e75] px-5 py-3 rounded-lg shadow-sm transition-colors">
              <svg class="w-6 h-6 text-[#003157] group-hover:text-white mr-3" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div class="grid gap-4">
            <div
              class="p-4 bg-white rounded-lg shadow-sm border border-[#dee3e9] hover:border-[#448ba9] transition-colors flex items-center">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-[#104e75] overflow-hidden h-[720px]">
          ...
          <div class="calendly-inline-widget" data-url="https://calendly.com/citafullness/30min"
    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="text-center mb-8 space-y-3">
        <h2 class="text-3xl md:text-4xl font-bold text-[#104e75]">
          ...
          <span class="bg-gradient-to-r from-[#003157] to-[#104e75] bg-clip-text text-transparent">Especialistas
            Certificados</span>
        </h2>
      ...
      <div class="flex flex-col lg:flex-row items-center justify-center gap-8 xl:gap-12">
        ...
        <div class="relative w-full max-w-[360px] aspect-[9/16] group -mt-[18px]">
          <div
            class="absolute inset-0 bg-gradient-to-t from-[#003157]/20 via-transparent to-transparent rounded-xl z-10 pointer-events-none">
          </div>
          <div
            class="absolute inset-0 border-2 border-white/20 rounded-xl group-hover:border-[#b7ebfa] transition-all duration-300 pointer-events-none">
          </div>

          <iframe src="https://www.youtube.com/embed/_vYxrOQpGVg"
            class="w-full h-full rounded-xl shadow-xl transform group-hover:scale-[1.02] transition-transform"
            title="Plataforma de fisioterapia domiciliaria" frameborder="0"
        ...
        <div class="space-y-8 max-w-2xl w-full lg:pl-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div class="p-5 bg-white rounded-lg border border-[#dee3e9] shadow-xs hover:shadow-sm transition-all">
              ...
              <p class="text-[#448ba9] leading-tight text-sm">25+ profesionales activos con diferentes especializaciones
              </p>
            </div>
  ...
  <section class="relative  /*bg-gradient-to-b from-[#dee3e9] to-white*/ bg-white py-16 px-4 sm:px-6"
    aria-label="Comunidades en línea">
    <div class="max-w-7xl mx-auto">
      ...
      <div class="absolute top-0 left-0 w-full overflow-hidden transform -translate-y-1">
        <svg class="relative block w-full h-16 text-[#dee3e9]" viewBox="0 0 1440 140" preserveAspectRatio="none">
          <path fill="currentColor" fill-opacity="0.1"
            d="M0,96L48,90.7C96,85,192,75,288,80C384,85,480,107,576,106.7C672,107,768,85,864,74.7C960,64,1056,64,1152,74.7C1248,85,1344,107,1392,117.3L1440,128L1440,0L1392,0C1344,0,1248,0,1152,0C1056,0,960,0,864,0C768,0,672,0,576,0C480,0,384,0,288,0C192,0,96,0,48,0L0,0Z">
          </path>
        </svg>
      </div>
      ...
      <div class="flex justify-between flex-nowrap gap-4 mb-8 overflow-x-auto pb-3 scrollbar-hide">
        <!-- Tarjeta 1 - Comunidad -->
        <div class="w-full max-w-[90%] md:max-w-none mx-auto bg-white rounded-xl p-4 md:p-6 my-8 border-2 border-[#488fad]">
          <!-- Mobile layout -->
          <div class="flex flex-col items-center gap-4 md:hidden w-full">
            <div class="relative w-24 h-24 shrink-0">
              <img
                src="https://i.ytimg.com/vi/u3udKyHY8AI/oar2.jpg?sqp=-oaymwEYCJUDENAFSFqQAgHyq4qpAwcIARUAAIhC&rs=AOn4CLCSskU6MFnDkglo_F1JqrdZjMsHng"
                alt="Perfil de comunidad" 
                class="w-full h-full rounded-full object-cover" 
              />
            </div>
            <div class="text-center">
              <h3 class="text-xl font-semibold text-[#104e75] leading-tight">
                ¿Te gustaría hacer del bienestar un habito, pero en comunidad?
              </h3>
            </div>
            <div class="w-full">
              <a href="#"
                class="flex items-center justify-center gap-2 bg-[#488fad] hover:bg-[#3a7186] text-white px-8 py-3 rounded-full font-bold transition-all duration-300 w-full">
                Ir a la comunidad
          ...
          <div class="hidden md:flex items-center gap-6 w-full">
            ...
            <div class="flex-1">
              <h3 class="text-3xl font-semibold text-[#104e75] leading-tight">
                ¿Te gustaría hacer del bienestar un habito, pero en comunidad?
              </h3>
            </div>
            <div class="ml-auto">
              <a href="https://chat.whatsapp.com/CuZQRXhqDxGBz7NpDhmH5F"
                target="_blank"
                class="flex items-center justify-center gap-2 bg-[#488fad] hover:bg-[#3a7186] text-white px-8 py-3 rounded-full font-bold transition-all duration-300 w-[230px]">
                Ir a la comunidad
                <i class="fa-brands fa-whatsapp text-white text-2xl"></i>
              </a>
            </div>
  ...
  <section
    class="relative bg-gradient-to-b from-[#dee3e9] to-white py-20 px-4 sm:px-6 lg:px-8 min-h-[800px] flex items-center">
    <div class="max-w-7xl mx-auto w-full">
      ...
      <div class="absolute top-0 left-0 w-full overflow-hidden transform -rotate-180">
        <svg class="relative block w-full h-[68px]" viewBox="0 0 1440 140" preserveAspectRatio="none">
          <path fill="#104e75" fill-opacity="0.15"
            d="M0,96L48,90.7C96,85,192,75,288,80C384,85,480,107,576,106.7C672,107,768,85,864,74.7C960,64,1056,64,1152,74.7C1248,85,1344,107,1392,117.3L1440,128L1440,0L1392,0C1344,0,1248,0,1152,0C1056,0,960,0,864,0C768,0,672,0,576,0C480,0,384,0,288,0C192,0,96,0,48,0L0,0Z">
          </path>
        </svg>
      </div>
      ...
      <div class="max-w-3xl mx-auto space-y-4 flex-grow">
        <!-- Item 1 -->
        <div class="border rounded-xl bg-white transition-all duration-200" :class="{ 'shadow-md': activeFaq === 1 }">
          <button @click="toggleFaq(1)" class="w-full px-5 py-4 text-left flex items-center justify-between group">
            ...
            <svg class="w-6 h-6 text-[#003157] transform transition-transform duration-200"
              :class="{ 'rotate-180': activeFaq === 1 }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div v-show="activeFaq === 1" class="px-5 pb-4 pt-0">
              <div class="space-y-3 text-gray-600 text-sm border-t border-[#b7ebfa] pt-4">
                ...
                <div class="mt-3 p-3 bg-[#dee3e9] rounded-lg border border-[#b7ebfa]">
                  <p class="text-sm font-medium text-[#003157]">Promociones: Descuentos en paquetes de sesiones y
                    beneficios por referidos.</p>
                </div>
        ...
        <!-- Item 2 -->
        <div class="border rounded-xl bg-white transition-all duration-200" :class="{ 'shadow-md': activeFaq === 2 }">
          <button @click="toggleFaq(2)" class="w-full px-5 py-4 text-left flex items-center justify-between group">
            <h3 class="text-lg font-semibold text-gray-800 pr-2">¿Cuánto dura cada sesión?</h3>
            <svg class="w-6 h-6 text-[#003157] transform transition-transform duration-200"
              :class="{ 'rotate-180': activeFaq === 2 }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          <transition name="faq">
            <div v-show="activeFaq === 2" class="px-5 pb-4 pt-0">
              <div class="space-y-2 text-gray-600 text-sm border-t border-[#b7ebfa] pt-4">
                ...
                <div class="mt-2 p-3 bg-[#dee3e9] rounded-lg border border-[#b7ebfa]">
                  <p class="text-sm text-[#003157]">Nuestro equipo se enfoca en brindar una atención completa y efectiva
                    en cada sesión.</p>
                </div>
              </div>
            </div>
        ...
        <!-- Item 3 -->
        <div class="border rounded-xl bg-white transition-all duration-200" :class="{ 'shadow-md': activeFaq === 3 }">
          <button @click="toggleFaq(3)" class="w-full px-5 py-4 text-left flex items-center justify-between group">
            <h3 class="text-lg font-semibold text-gray-800 pr-2">¿Qué métodos de pago aceptan?</h3>
            <svg class="w-6 h-6 text-[#003157] transform transition-transform duration-200"
              :class="{ 'rotate-180': activeFaq === 3 }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          <transition name="faq">
        ...
        <!-- Item 4 -->
        <div class="border rounded-xl bg-white transition-all duration-200" :class="{ 'shadow-md': activeFaq === 4 }">
          <button @click="toggleFaq(4)" class="w-full px-5 py-4 text-left flex items-center justify-between group">
            <h3 class="text-lg font-semibold text-gray-800 pr-2">¿Trabajan con seguros de salud?</h3>
            <svg class="w-6 h-6 text-[#003157] transform transition-transform duration-200"
              :class="{ 'rotate-180': activeFaq === 4 }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          <transition name="faq">
            <div v-show="activeFaq === 4" class="px-5 pb-4 pt-0">
              <div class="space-y-2 text-gray-600 text-sm border-t border-[#b7ebfa] pt-4">
                <p>Actualmente estamos en proceso de alianzas con aseguradoras. Escríbenos para verificar cobertura con
                  tu
                  seguro.</p>
                <div class="mt-2 p-3 bg-[#dee3e9] rounded-lg border border-[#b7ebfa]">
        ...
        <!-- Item 5 -->
        <div class="border rounded-xl bg-white transition-all duration-200" :class="{ 'shadow-md': activeFaq === 5 }">
          <button @click="toggleFaq(5)" class="w-full px-5 py-4 text-left flex items-center justify-between group">
            <h3 class="text-lg font-semibold text-gray-800 pr-2">¿Puedo cancelar o reprogramar una sesión?</h3>
            <svg class="w-6 h-6 text-[#003157] transform transition-transform duration-200"
              :class="{ 'rotate-180': activeFaq === 5 }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          <transition name="faq">
            <div v-show="activeFaq === 5" class="px-5 pb-4 pt-0">
              <div class="space-y-2 text-gray-600 text-sm border-t border-[#b7ebfa] pt-4">
                ...
                <div class="mt-2 p-3 bg-[#dee3e9] rounded-lg border border-[#b7ebfa]">
                  <p class="text-sm text-[#003157]">Para cambios de última hora, comunícate con nosotros inmediatamente.
                  </p>
                </div>
              </div>
            </div>
        ...
        <!-- Item 6 -->
        <div class="border rounded-xl bg-white transition-all duration-200" :class="{ 'shadow-md': activeFaq === 6 }">
          <button @click="toggleFaq(6)" class="w-full px-5 py-4 text-left flex items-center justify-between group">
            <h3 class="text-lg font-semibold text-gray-800 pr-2">¿Los fisioterapeutas están certificados?</h3>
            <svg class="w-6 h-6 text-[#003157] transform transition-transform duration-200"
              :class="{ 'rotate-180': activeFaq === 6 }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
            </svg>
          </button>
          <transition name="faq">
            <div v-show="activeFaq === 6" class="px-5 pb-4 pt-0">
              <div class="space-y-2 text-gray-600 text-sm border-t border-[#b7ebfa] pt-4">
                <p>Todos nuestros fisioterapeutas están certificados y colegiados, con experiencia en diversas
                  especialidades.</p>
                <div class="mt-2 p-3 bg-[#dee3e9] rounded-lg border border-[#b7ebfa]">
                  <p class="text-sm text-[#003157]">Garantizamos tratamientos seguros y efectivos con atención
                    personalizada.</p>
                </div>
              </div>
            </div>
      ...
      <div class="mt-12 text-center space-y-3">
        ...
        <button
          class="bg-gradient-to-r from-[#104e75] to-[#003157] text-white px-6 py-2.5 rounded-full text-sm font-medium hover:from-[#003157] hover:to-[#104e75] transition-all inline-flex items-center gap-2 transform hover:scale-[1.02] active:scale-95">
          Contáctanos ahora
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6 py-6 border-b border-[#b7ebfa]/20">
        ...
        <div class="space-y-2 md:space-y-3">
          <div class="flex items-center gap-2">
            <img src="@/assets/imagenes/LOGO.svg" alt="Fullness"
        ...
        <div class="space-y-2 md:space-y-3">
          ...
          <div class="grid grid-cols-3 gap-1 md:gap-2">
            <a href="https://wa.me/51987654321" target="_blank"
              class="p-1.5 md:p-2 bg-[#003157]/30 hover:bg-[#104e75]/40 rounded-md flex flex-col items-center transition-all group">
              <svg class="w-5 h-5 text-[#448ba9] group-hover:text-[#b7ebfa] mb-1" fill="currentColor"
                viewBox="0 0 24 24">
                <path
